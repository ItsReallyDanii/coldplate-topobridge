...
| U-013 | Missing upstream control artifacts | NOT_PROVEN | Blocks VALIDATED tier |
...
**Description:** The bridge VALIDATED tier requires G5 (negative control on uniform geometry) and G6 (positive control on known varied geometry). The upstream parent repo `coldplate-design-engine` does NOT currently emit a Stage 4 artifact for a uniform channel `baseline` or another explicitly calibrated control.

**Impact:** The bridge real-artifact path is blocked at IMPLEMENTED and cannot be advanced to VALIDATED until identical-solver Stage 4 artifacts are generated for control geometries. 

**Resolution path:** Upstream requirement: Generate and commit to `results/stage4_sim_full/` at least one uniform-channel artifact (e.g., from `baselines/channels/`) using the exact same solver as the diamond TPMS candidates.

**NOT claimed:** That the existing synthetic controls are sufficient to validate the Stage 4 real-data path.
